...
| [enumerateNetworkDevices](functions/enumerateNetworkDevices.md) | Find network printers on the local subnets: one SNMP broadcast of `hrDeviceDescr.1`, every responder matched against the TCP-capable entries of the given registries, then one unicast for the serial. Responders without `hrDeviceDescr.1` go through [identifyNetworkDevice](functions/identifyNetworkDevice.md) for the `sysDescr` fallback. |
